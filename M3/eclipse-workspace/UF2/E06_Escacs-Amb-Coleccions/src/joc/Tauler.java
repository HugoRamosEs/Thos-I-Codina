...
//	public boolean[][] pecesDelMateixEquip(MyColor col) {
...
//	}
	...
	public HashSet<Posicio> pecesDelMateixEquip(MyColor col) {
		HashSet<Posicio> pecesDelMateixEquip = new HashSet<Posicio>();
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if ((this.escacs[i][j] instanceof AmbPeca) &&
					(((AmbPeca)this.escacs[i][j]).getPeca().isMateixEquip(col)))
...
//	public boolean[][] pecesEquipContrari(MyColor col) {
...
//	}
	...
	public HashSet<Posicio> pecesEquipContrari(MyColor col) {
		HashSet<Posicio> pecesEquipContrari = new HashSet<Posicio>();
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				if ((this.escacs[i][j] instanceof AmbPeca) &&
					!(((AmbPeca)this.escacs[i][j]).getPeca().isMateixEquip(col)))
...
//	}
	...
	public void imprimirMateix(MyColor col) {
		...
		for (int j = 7; j >= 0; j--) {
			...
			for (int i = 0; i < 8; i++) {
				if (this.escacs[i][j] != null) {
					String aux = (peces.contains(new Posicio(i,j)))?" * ":" O ";
					System.out.print("|(" + aux + ")|");
				}
