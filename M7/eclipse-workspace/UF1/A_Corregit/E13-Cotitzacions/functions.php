...
function html_generateTable($aParametre) {
    ...
    foreach ($aParametre as $key => $value) {
        ...
        foreach ($value as $clau => $valor) {
            //La primera vegada no hi ha $result, per tant carrego també la capcelera
            if (!isset($resultat)) {
                $capcelera .= "<th>".ucwords($clau)."</th>";
            }
            if ($clau == "ultima_coti") {
                //Verifico si hi ha sessió
                if (isset($_SESSION["cotis"])) {
                    //Si hi ha sessió.... converteixo l'string a float i comparo
                    //si eś més gran o més petit que l'anterior valor $_SESSION
                    ...
                    //Si no hi ha sessió poso directament sense class
                    $cos .= "<td>$valor</td>";
                }
