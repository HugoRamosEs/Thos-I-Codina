...
public class E03_SumaNumeros {
	...
	public static void main(String[] args) throws IOException {
        ...
        RandomAccessFile dades = programa.openLectura("enters.dat");
        if (dades == null) {
            return;
        }
        int suma = 0;
		while (dades.getFilePointer() < dades.length()) {
			suma = suma + dades.readInt();
		}
		
		RandomAccessFile sumaTotal = programa.openEscriptura("numeros.dat");
	    if (sumaTotal == null) {
	    	   return;
	    }
	    sumaTotal.setLength(0);
		  
	    sumaTotal.writeInt(suma);
