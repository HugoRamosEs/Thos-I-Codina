...
class Controller {
    ...
    public function menuIdiomes($lang){
        $idiomaModel = new IdiomaModel();
        $idiomesActius = $idiomaModel->getActiveLangs();
        $langTraduccions = [];
        $isos = [];
        $iS = null;
        foreach ($idiomesActius as $iA){
            $isos[] = $iA->getIso();
            $langTraduccions[$iA->getIso()]["iso"] = $iA->getIso();
            $langTraduccions[$iA->getIso()]["imatge"] = $iA->getImatge();
            if ($iA->getIso() == $lang){
                $iS = $iA;
            }
        }
        if ($iS !== null) {
            foreach ($iS->getTraduccions() as $t) {
                foreach ($isos as $i) {
                    if ($i == $t->getVariable()) {
                        $langTraduccions[$i]["valors"][] = $t->getValor();
                    }
                }
            }
        }
        foreach ($langTraduccions as &$trad) {
            if (isset($trad["valors"])) {
                $trad["valors"] = array_reverse($trad["valors"]);
            }
        }
        
        return $langTraduccions;
    }
}
